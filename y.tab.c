...
vector<SymbolInfo*>v;
vector< pair<string,string> >decld_var_carrier;   
vector< pair<string,string> >var_carrier;
vector< pair<string,string> >decld_f_var;         
...
{
       0,   130,   130,   164,   169,   175,   180,   184,   191,   250,
     281,   281,   432,   432,   522,   529,   536,   544,   552,   552,
     583,   583,   614,   633,   641,   648,   657,   677,   699,   720,
     747,   753,   761,   765,   770,   775,   794,   809,   827,   844,
     851,   867,   871,   879,   910,   950,   955,  1004,  1012,  1041,
    1050,  1090,  1098,  1137,  1145,  1242,  1260,  1277,  1286,  1302,
    1371,  1382,  1393,  1404,  1419,  1436,  1451,  1456,  1475
};
#endif
...
#line 130 "Parser.y"
	...
	if(numberOfErrors == 0){
		...
		(yyvsp[0].symbolInfoPointer)->extraSymbolInfo.assm_code = temp + "\n.CODE" + output_procedure + (yyvsp[0].symbolInfoPointer)->extraSymbolInfo.assm_code;
...
#line 281 "Parser.y"
		...
		if(s==0){
			...
			if(s->extraSymbolInfo.isFunctionDefined){
				...
				if(actReturnType != declaredReturnType){
					...
					for(int i =0;i<temp_param_list.size();i++)
					{
						s->extraSymbolInfo.modfd_param_list.push_back(temp_param_list[i].first+to_string(scope_counter));               
					}
				}
		...
		running_f_name = (yyvsp[-3].symbolInfoPointer)->getName(); 
...
#line 432 "Parser.y"
	...
	running_f_name = (yyvsp[-2].symbolInfoPointer)->getName();
	decld_var_carrier.push_back(make_pair(running_f_name+"_return_val", ""));
}
#line 1817 "y.tab.c"
    break;
...
#line 910 "Parser.y"
	...
	if(temp!=0){
		...
		if(typeOfID!="ARRAY"){
			...
			string type = (yyvsp[-1].symbolInfoPointer)->extraSymbolInfo.typeOfVar; 
